...
airPressureRouter.get('/get/per-hour/outside', async (c: any) => {
    const result = await airPressureController.getOutsideAirPressureHandler(span.default.hour);
    return c.json(result);
});
airPressureRouter.get('/get/per-day/outside', async (c: any) => {
    const result = await airPressureController.getOutsideAirPressureHandler(span.default.day);
    return c.json(result);
});
airPressureRouter.get('/get/per-week/outside', async (c: any) => {
    const result = await airPressureController.getOutsideAirPressureHandler(span.default.week);
    return c.json(result);
});
airPressureRouter.get('/get/per-month/outside', async (c: any) => {
    const result = await airPressureController.getOutsideAirPressureHandler(span.default.month);
    return c.json(result);
});
